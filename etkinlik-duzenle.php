...
if(!empty($_GET["ID"]))
{
	...
	$kontrol=$DB->VeriGetir("etkinlikler","","","ORDER BY ID ASC");
	

	if($kontrol!=false)
	{
		$veri=$DB->VeriGetir("etkinlikler","WHERE ID=?",array($ID),"ORDER BY ID ASC");
		if($veri!=false)
		{			
	...
	<div class="content-wrapper">
        ...
        <div class="content-header">
			<div class="container-fluid">
				<div class="row mb-2">
					<div class="col-sm-6">
						<h1 class="m-0">Etkinlik Güncelleme</h1>
					</div>
		    ...
		    <div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<a href="<?=SITE?>etkinlik-liste" class="btn btn-info" style="float:right; margin-left:10px;"><i style="margin-right:3px;"class="fas fa-bars"> LİSTELE</i></a>
		...
		if($_POST)
		{
			...
			if((!empty($_POST["baslik"])) && (!empty($_POST["aciklama"])) )
			{
			    ...
			    if(!empty($_FILES["resim"]["name"]))
				{
					...
					if($yukle!=false)
					{
						$guncelle=$DB->SorguCalistir("UPDATE etkinlikler","SET Baslik=?,Aciklama=?,Resim=?, Tarih=?	WHERE ID=?",
				...
				{
			        $guncelle=$DB->SorguCalistir("UPDATE etkinlikler","SET Baslik=?,Aciklama=?,Tarih=?	WHERE ID=?",
			        array($baslik,$aciklama,date("Y-m-d"),$veri[0]["ID"]));
						  
				}
				if($guncelle!=false)
				{
					$veri=$DB->VeriGetir("etkinlikler","WHERE ID=?",array($veri[0]["ID"]),"ORDER BY ID ASC"); 
					?>
