...
if(!empty($_GET["ID"]))
{
	...
	$kontrol=$DB->VeriGetir("muhtarlikgorev","","","ORDER BY ID ASC");
	

	if($kontrol!=false)
	{
		$veri=$DB->VeriGetir("muhtarlikgorev","WHERE ID=?",array($ID),"ORDER BY ID ASC");
		if($veri!=false)
		{			
	...
	<div class="content-wrapper">
        ...
        <div class="content-header">
			<div class="container-fluid">
				<div class="row mb-2">
					<div class="col-sm-6">
						<h1 class="m-0">Muhtar Görevleri Güncelle</h1>
					</div>
					
					<div class="col-sm-6">
						<ol class="breadcrumb float-sm-right">
						<li class="breadcrumb-item"><a href="<?=SITE?>">Anasayfa</a></li>
						<li class="breadcrumb-item active">Muhtar Görevleri</li>
						</ol>
					</div>
				</div>
		    ...
		    <div class="container-fluid">
				...
				<div class="row">
					<div class="col-md-12">
						...
						<a href="<?=SITE?>muhtargorev-liste" class="btn btn-info" style="float:right; margin-left:10px;"><i style="margin-right:3px;"class="fas fa-bars"> LİSTELE</i></a>
	...
	<?php
		if($_POST)
		{
			...
			if((!empty($_POST["baslik"])) && (!empty($_POST["aciklama"])) )
			{
				...
				{
					$yukle=$DB->upload("resim","images/MuhtargorevFotograflar");
					error_reporting(0); 
					...
					if($yukle!=false)
					{
						$guncelle=$DB->SorguCalistir("UPDATE muhtarlikgorev","SET Baslik=?,Aciklama=?,Resim=? WHERE ID=?",
			            array($baslik,$aciklama,$yukle,$veri[0]["ID"]),1);
				...
				{
			        $guncelle=$DB->SorguCalistir("UPDATE muhtarlikgorev","SET Baslik=?,Aciklama=? WHERE ID=?",
			        array($baslik,$aciklama,$veri[0]["ID"]));
						  
				}
				if($guncelle!=false)
				{
					$veri=$DB->VeriGetir("muhtarlikgorev","WHERE ID=?",array($veri[0]["ID"]),"ORDER BY ID ASC"); 
					?>
